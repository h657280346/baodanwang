...
<body>
                            ...
                            <div class="ibox-content">
                                ...
                                <table class="table table-striped table-bordered table-hover dataTables-example">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach items="${rewards}"  var="item" varStatus="status">
                                        <tr class="gradeX">
                                            <td class="center" style="align-items: center"> <input type="checkbox" name="del" value="${item.id }" style="align:center"/></td>
                                            <td><a href="./findRewardById.action?id=${item.award }">${item.award}</a></td>
	...
	<script type="text/javascript">
		function add(){
		  if('${requestScope.rewards[0].id}'==null || '${requestScope.rewards[0].id}'=="" ){
		 	 window.location.href="./skipReward.action";
		  }	 else {
		  	alert("已添加!");
		  }
		}
		function update(id){
			window.location.href="./findRewardById.action?id="+id;
		}
		function dele(id){
			if(confirm("此操作不可逆，是否继续执行？"))
			window.location.href="./rewardDelete.action?id="+id;
		}
		function del(){
			if(confirm("此操作不可逆，是否继续执行？")){
