...
<body>
                        ...
                        <div class="ibox float-e-margins">
                            <div class="ibox-content">
                                ...
                                <table class="table table-striped table-bordered table-hover dataTables-example">
                                    ...
                                    <tbody>
                                        <c:forEach items="${requestScope.plan}"  var="item" varStatus="status">
                                        <tr class="gradeX">
                                            <td class="center" style="align-items: center"> <input type="checkbox" name="del" value="${item.id }" style="align:center"/></td>
                                            <td><a href="./findplanById.action?id=${item.id }">${item.company}</a></td>
                                            <td>${item.time }</td>
                                            <td class="center" style="width:30%"><input type="button" class="btn btn-w-m btn-primary" style="margin-right: 20px" value="查看详情" onclick="update('${item.id }')"/><input type="button" class="btn btn-w-m btn-danger" value="删除" onclick="dele('${item.id }')"/></td>
                                        </tr>
										</c:forEach>
                                    </tbody>
                                    <tfoot>
	...
	<script type="text/javascript">
		function add(){
			window.location.href="./skipPlan.action";
		}
		function update(id){
			window.location.href="./findplanById.action?id="+id;
		}
		function dele(id){
			if(confirm("此操作不可逆，是否继续执行？"))
			window.location.href="./planDelete.action?id="+id;
		}
		function del(){
			if(confirm("此操作不可逆，是否继续执行？")){
