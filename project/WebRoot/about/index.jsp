...
<body>
	...
	<script type="text/javascript">
		function add(){
		  if('${requestScope.abouts[0].id}'==null || '${requestScope.abouts[0].id}'=="" ){
		 	 window.location.href="./skipAbout.action";
		  }	 else {
		  	alert("已添加!");
		  }
		}
		function update(id){
			window.location.href="./findAboutById.action?id="+id;
		}
		function dele(id){
			if(confirm("此操作不可逆，是否继续执行？"))
			window.location.href="./aboutDelete.action?id="+id;
		}
		function del(){
			if(confirm("此操作不可逆，是否继续执行？")){
