...
<body>
                            ...
                            <div class="ibox-content">
                                ...
                                <table class="table table-striped table-bordered table-hover dataTables-example">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach items="${players}"  var="item" varStatus="status">
                                        <tr class="gradeX">
											...
											<td>${item.status }</td>
											<c:if test="${item.identity=='p'}"><td>发布球队</td></c:if>
											<c:if test="${item.identity=='m'}"><td>约战球队</td></c:if>
                                            <td>${item.time } </td>
                                            <td class="center" style="width:15%"><input type="button" class="btn btn-primary" style="margin-right: 20px" value="详情" onclick="update('${item.id }')"/><input type="button" class="btn  btn-danger" value="删除" onclick="dele('${item.id }')"/></td>
                                        </tr>
										</c:forEach>
                                    </tbody>
                                    <tfoot>
	...
	<script type="text/javascript">
		function add(){
			window.location.href="./skipPlayer.action";
		}
		function update(id){
			window.location.href="./findPlayerById.action?id="+id;
		}
		function dele(id){
			if(confirm("此操作不可逆，是否继续执行？"))
			window.location.href="./playerDelete.action?id="+id;
		}
		function del(){
			if(confirm("此操作不可逆，是否继续执行？")){
