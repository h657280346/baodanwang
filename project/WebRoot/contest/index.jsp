...
<body>
                            ...
                            <div class="ibox-content">
                                ...
                                <table class="table table-striped table-bordered table-hover dataTables-example">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach items="${contests}"  var="item" varStatus="status">
                                        <tr class="gradeX">
                                            <td class="center" style="align-items: center"> <input type="checkbox" name="del" value="${item.id }" style="align:center"/></td>
                                            <td><a style="word-break: break-all;">${item.popenid}</a></td>
                                            <td>${item.pteamid}</td>
                                            <c:if test="${item.mopenid==''}"><td>无</td></c:if>
                                            <c:if test="${item.mopenid!=''}" ><td style="word-break: break-all;"><a>${item.mopenid}</a></td></c:if>
                                            <td>${item.mteamid==''?"无":item.mteamid}</td>
                                            <td style="word-break: break-all;">${item.date }&nbsp;&nbsp;${item.time }</td>
                                             <td style="word-break: break-all;">${item.postdate }</td>
											<td><a style="word-break: break-all;">${item.adress }</a></td>
                                            <td>${item.contact }</td>
                                            <td>${item.person } &nbsp;人</td>
                                            <td>${item.cost }</td>
                                            <td style="word-break: break-all;">${item.status }</td>
                                            <td class="center" style="width:5%"><input type="button" class="btn  btn-primary" style="margin-right: 10px" value="详情" onclick="update('${item.id }')"/><input type="button" class="btn btn-danger" value="删除" onclick="dele('${item.id }')"/></td>
                                        </tr>
										</c:forEach>
                                    </tbody>
                                    <tfoot>
	...
	<script type="text/javascript">
		function add(){
			window.location.href="./skipContest.action";
		}
		function update(id){
			window.location.href="./findContestById.action?id="+id;
		}
		function dele(id){
			if(confirm("此操作不可逆，是否继续执行？"))
			window.location.href="./contestDelete.action?id="+id;
		}
		function del(){
			if(confirm("此操作不可逆，是否继续执行？")){
